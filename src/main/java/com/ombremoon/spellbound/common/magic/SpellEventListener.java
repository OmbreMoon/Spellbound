...
public class SpellEventListener {
    private final Multimap<Event<?>, EventInstance<? extends SpellEvent>> events = ArrayListMultimap.create();
    ...
    public <T extends SpellEvent> void addListener(Event<T> event, UUID uuid, Consumer<T> consumer) {
    ...
    private boolean isProperSide(Event<?> event) {
        return this.player.level().isClientSide == event.isClientSide();
    }

    @SuppressWarnings("unchecked")
    public <T extends SpellEvent> boolean fireEvent(Event<T> event, T spellEvent) {
    ...
    public static class Event<T extends SpellEvent> {
        public static Event<PlayerJumpEvent> JUMP = new Event<>(false);
